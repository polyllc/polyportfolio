<script>
    ...
    let polylogo = `<img src="src/assets/polytiles2.png" width="50" height="50" class="githubLogo">`
    ...
    $: overlayText = css`
    color: white; text-decoration: underline; text-decoration-color: #fbc200; font-weight: 800;
    margin: 0;
    ...
    let c = Math.floor(Math.random() * 125) + 20;

    let randomBorder = css`
        &:hover {
    ...
    $: changeOverlayBG = css`background: linear-gradient(${135}deg, #252f89,#252f89,#101c7f, rgba(0,0,0,0) ${120 * clamp(1 - window.scrollY / 400, 0, 1)}%);`;
    ...
    onscroll = (event) => {
        ...
        changeOverlayText = css`
        font-size: ${clamp(150 + window.scrollY * 4, 150, 2000)}px !important; 
        opacity: ${scrollValue};
        background-clip: text;
    -webkit-text-fill-color: transparent;
        -webkit-background-clip: text;
        `;
        changeOverlayBG = css`background: linear-gradient(135deg, #252f89,#252f89,#101c7f,#212b6c,#262b6c,#24319d 0%, rgba(0,0,0,${scrollValue}) ${120 * clamp(1 - window.scrollY / 400, 0, 1)}%);
        `;
...
<div style="background: linear-gradient(135deg, #252f49,#101c3f,#212b3c, #24313d 0%, rgba(0,0,0,1) 120%);">
...
<div class="boxHolder {randomBorder}">
    <Box title={"about"} date={"2017-\n2024"} description="Welcome to my portfolio! This is a collection of works from years 2017 to 2024. They vary in quality as well as in scale, but I do hope you look around." 
        icon={polylogo} language={languages[7]} link={"/"} extraCss={"width: 300% !important; max-width: 999% !important;"}/>
    <div class="boxDivider">
...
<div id="mainContent">
    <div id="leftMain">
        <p style="font-size: 150px; color: white; text-decoration: underline; text-decoration-color: #fbc200; justify-content:center; font-weight: 800;">About</p>
    </div>
    <div id="rightMain">
        <p style="font-size: 25px; color: white; margin-left: 5%; margin-top: 2%; margin-right: 20%; font-weight: 0; text-shadow: 0 0 15pt black; line-height: 160%;">
            &emsp;Hey! My name is Poly, and welcome to my portfolio! This portfolio is a collection of projects that I have finished (or worked on) over the years since 2017. I started to program when I was eight in Scratch. My dad was watching a TED talk
